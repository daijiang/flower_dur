...
plt_summary = function(us_map = readRDS("data/usa_map.rds"), 
                       ...
                       show_fig = FALSE){
  # make grid over us
  xsf = st_make_grid(us_map, cellsize = c(cell_size, cell_size))
  # add grid cell id
  xsf = mutate(st_sf(geometry = xsf), id_cells = 1:n())
  ...
  # cells with data
  cells_with_data = dplyr::filter(xsf, id_cells %in% dat_cells_count$id_cells) %>% 
  ...
  plt = ggplot() +
    geom_sf(data = us_map) +
    geom_sf(data = xsf, alpha = 0, size = 0.1, color = "gray") +
    geom_sf(data = dat, size = 0.5, alpha = 0.6) +
    geom_sf(data = filter(cells_with_data, enough_data), alpha = 0, size = 0.15, color = "red")
  print(plt)
  cat(nrow(filter(cells_with_data, enough_data)), "cells with records more than", n_per_cell, "\n")
  
  list(cells_with_data = cells_with_data, fig = plt)
}
...
cell_10k = plt_summary(cell_size = 20000, dat = d, n_per_cell = 10)
